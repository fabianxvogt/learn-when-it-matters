...
Classification: **EXPLORATORY**. The mechanism is a narrow empirical hypothesis in a crowded test-time-learning/surprise-gating setting; the project makes no novelty claim. The final measured-cost scientific attempt is closed as bounded inconclusive; product review and deployment remain separate.
...
The usefulness gate may simply be a delayed heuristic that cannot outperform tuned controls once shadow-probe compute, state copies, rejected work and elapsed time are charged. A complete negative result would stop architecture growth, but the closed run had no strict comparison and does not establish that result.
...
The final investigation adds measured CPU/wall calibration, common-bin validity,
complete paired held-out rows, and an explicit inconclusive path. Its output may
support only a bounded exploratory result for the declared seeds, stream cells,
models and host; it cannot establish inferiority, equivalence, universal
usefulness gating or a hardware-independent theorem. If strict bins have no
intersection, its separate upper-cost ceiling table keeps cheaper controls
...
## Train-only measurement amendment and closure
...
external watchdog may not exceed 1,800 seconds. The one authorized amended run
is consumed and independently interpreted as bounded **INCONCLUSIVE measurement
identification**: 102 calibration rows, zero strict held-out rows, and 672
partial ceiling rows. The CV-instability exclusions in the linear model and
the non-overlapping accepted timing ranges in the recurrent model prevent a
strict common bin. The ceiling rows are descriptive only and cannot identify a
best method. No new pilot, benchmark, calibration, tuning, architecture
expansion, model-win, equivalence, inferiority, negative-hypothesis, or full-v1
